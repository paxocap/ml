...
int main()
{
 ...
 //logic to compute log likelihood of unigram model
  ...
  plogunigram = plogunigram * (vocabuprob.find("AGENCY")->second);
  plogunigram = plogunigram * (vocabuprob.find("OFFICIALS")->second);
  plogunigram = plogunigram * (vocabuprob.find("SOLD")->second);
  plogunigram = plogunigram * (vocabuprob.find("SECURITIES")->second);
 ...
 //logic to compute log likelihood of bigram model
  ...
  plogbigram = plogbigram * pb[(vocabindex.find("THE")->second)][(vocabindex.find("AGENCY")->second)];
  plogbigram = plogbigram * pb[(vocabindex.find("AGENCY")->second)][(vocabindex.find("OFFICIALS")->second)];
  plogbigram = plogbigram * pb[(vocabindex.find("OFFICIALS")->second)][(vocabindex.find("SOLD")->second)];
  plogbigram = plogbigram * pb[(vocabindex.find("SOLD")->second)][(vocabindex.find("SECURITIES")->second)];

  logbigram = log(plogbigram);  //this holds the log likelihood value of unigram model for the given sentence.
  cout<<"log likelihood val for bigram model if we ignore </s> at the end = "<<logbigram<<endl;
  ...
  plogbigram = plogbigram * pb[(vocabindex.find("SECURITIES")->second)][(vocabindex.find("</s>")->second)];
  logbigram = log(plogbigram);  //this holds the log likelihood value of unigram model for the given sentence.
  cout<<"log likelihood val for bigram model if we consider </s> at the end = "<<logbigram<<endl;
  output<<"log likelihood val for bigram model if we consider </s> at the end = "<<logbigram<<endl;
  cout<<"oops! it seems for missing entries in index count table in bigram model can lead to erraneous log likelihood value!"<<endl;
  output<<"oops! it seems for missing entries in index count table in bigram model can lead to erraneous log likelihood value!"<<endl;
